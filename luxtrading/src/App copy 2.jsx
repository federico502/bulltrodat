...
// --- CAMBIO 1: Configuración de Axios para producción ---
    ...
    (page = 1) => {
      if (isOpen) {
        axios
          ...
          .then((res) => {
            // Añadir un campo 'password' vacío a cada usuario para el input controlado
  ...
  const handleSave = useCallback(
    async (userData) => {
      ...
      try {
        ...
        fetchUsers(pagination.currentPage); // Recarga para confirmar datos desde el server
      } catch (error) {
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    // --- CAMBIO 2: Lógica dinámica para la URL del WebSocket ---
    const connectWebSocket = () => {
      const wsProtocol = window.location.protocol === "https:" ? "wss" : "ws";
      const wsHost = new URL(axios.defaults.baseURL).host;
