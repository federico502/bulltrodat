...
//================================================================================
const useBinanceWebSocket = (assets, setRealTimePrices) => {
  useEffect(() => {
    if (!assets || assets.length === 0) return;
    const streams = assets.map((s) => s.toLowerCase() + "@trade").join("/");
...
// --- Componente de un solo item del menú ---
...
// --- Componente de Menú de Perfil (Optimizado) ---
const ProfileMenu = React.memo(
  ({ user, logout, onToggleSideMenu, onManageUsers, onManageRegCode }) => {
    ...
    // Función para envolver los clics y cerrar el menú
...
const TradingViewWidget = React.memo(({ symbol }) => {
  ...
  const getTradingViewSymbol = (assetSymbol) => {
    const forexPairs = ["EURUSD", "USDJPY"];
    const commodities = ["XAGUSD"];

    if (!assetSymbol) return "BINANCE:BTCUSDT";

    if (assetSymbol.includes("USDT")) {
      return `BINANCE:${assetSymbol}`;
    }
    if (forexPairs.includes(assetSymbol) || commodities.includes(assetSymbol)) {
      return `OANDA:${assetSymbol}`;
    }
    return `NASDAQ:${assetSymbol}`;
  };
...
const LiveProfitCell = ({ operation }) => {
  ...
  const calculateProfit = useCallback(() => {
    ...
    const priceData = realTimePrices[operation.activo.toUpperCase()];
...
const NewOperationModal = ({ isOpen, onClose, operationData, onConfirm }) => {
  ...
  const priceData = asset ? realTimePrices[asset.toUpperCase()] : null;
...
// Componente para una sola tarjeta de usuario, memoizado para evitar re-renders.
const UserCard = React.memo(({ user, onViewUserOps, onDeleteUser, onSave }) => {
...
const ManageUsersModal = ({
  ...
  // Este es el UserRow para la tabla de escritorio
...
const SideMenu = React.memo(({ isOpen, onClose, setAlert }) => {
  const [view, setView] = useState("main"); // 'main', 'profile', 'deposit', 'withdraw'
...
const DashboardPage = () => {
    ...
    const connectWebSocket = () => {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.tipo === "operacion_cerrada") {
            setAlert({
  ...
  }, [user, fetchData, pagination.currentPage, opHistoryFilter]);

  useEffect(() => {
    localStorage.setItem("userTradingAssets", JSON.stringify(userAssets));
  }, [userAssets]);

  useBinanceWebSocket(userAssets, setRealTimePrices);
  ...
  useEffect(() => {
    ...
    const pnl = openOperations.reduce((total, op) => {
      const currentPriceData = realTimePrices[op.activo.toUpperCase()];
      const currentPrice = currentPriceData?.price || op.precio_entrada;
      return (
        total +
  ...
  const handleOpenNewOpModal = useCallback(
    (type, volume) => {
      ...
      const currentPriceData = realTimePrices[selectedAsset.toUpperCase()];
      ...
      const freeMargin = parseFloat(metrics.freeMargin);
      if (cost > freeMargin) {
        setAlert({
  ...
  const handleConfirmOperation = useCallback(
    async (opDetails) => {
      const priceData = realTimePrices[selectedAsset.toUpperCase()];
      const livePrice = priceData?.price;
      if (!livePrice) {
        setAlert({
  ...
  const handleOpRowClick = useCallback(
    (op) => {
      ...
      if (op.cerrada) {
        ...
        const priceData = realTimePrices[op.activo.toUpperCase()];
        const currentPrice = priceData?.price;
        profit = currentPrice
          ? op.tipo_operacion.toLowerCase() === "sell"
            ? (op.precio_entrada - currentPrice) * op.volumen
    ...
    setConfirmationModal({
      ...
      onConfirm: async () => {
        try {
          ...
          setIsUsersModalOpen(false); // Cierra el modal de usuarios para que se refresque al abrirlo de nuevo
        } catch (error) {
          setAlert({
            message:
